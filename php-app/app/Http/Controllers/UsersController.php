...
class UsersController extends Controller {
    ...
    /**
     * Function is to get data for project assigning
     * 
    ...
    public function apiIndex(Request $request)
    {
        $user_ids = [];
        if ($request->get('id')) {
            $user_ids = ProjectUser::where('site_id', $request->get('id'))->lists('user_id');
        }
        $search = $request->get('search');
        $users = UserModal::whereNotIn('id', $user_ids)
...
//        $roleList = [''=>''] + $roleCollection->all();
        ...
        return view('admin.users.show', compact('user', 'perms', 'page_title', 'page_description', 'companies'));
    }
    ...
    public function create()
    {
		...
		$random_password = bin2hex(openssl_random_pseudo_bytes(5));
        return view('admin.users.create', compact('user', 'perms', 'page_title', 'page_description', 'companies', 'random_password'));
    }
    ...
    public function store(CreateUserRequest $request)
    {
        ...
        Audit::log(Auth::user()->id, trans('admin/users/general.audit-log.category'), trans('admin/users/general.audit-log.msg-store', ['email' => $attributes['email']]));
		...
		$data = ['user' => $user, 'password' => $attributes['password'], 'URL' => 'https://'.env('APP_URL', 'login') ];

		Mail::send('emails.new-user', $data, function ($message) use ($user) {
		    $message->from('support@wisercapital.com', 'New User Account - Wiser Capital Support');
			$message->to($user->email, $user->first_name . ' ' . $user->last_name)->subject('Welcome to Wiser Capital Platform');
        });
    ...
    public function edit($id)
    {
        ...
        return view('admin.users.edit', compact('user', 'roles', 'perms', 'page_title', 'page_description', 'companies'));
    }
    ...
    public function update(UpdateUserRequest $request, $id)
    {
        ...
        // checking if a company was added for the user.
        // 1. user had a company and it was changed
        // 2. user had a company and it was removed
        /// 3. user didn't had a company and it was added
        if (($user->company_id != $attributes['company_id']) ) {
            CompanySite::where('company_id', $user->company_id)->delete();
    ...
    public function updateProjectListView(Request $request) 
    { 
	    
	    $user = Auth::user();
        $user->setMeta([
            'user_project_list_preferrence' => $request->get('view')
        ]);
        $user->save();
	...
	{
		$new_user = $this->user->find($user_id);
		Auth::login( $new_user );
